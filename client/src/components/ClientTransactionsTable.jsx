...
const deleteLedgerTransaction = async (transactionId) => {
  const token = getAuthToken();
  await axios.delete(`/api/transactions/ledger/${transactionId}`, {
    headers: { Authorization: `Bearer ${token}` }
  });
};
...
const ClientTransactionsTable = ({ portfolioId }) => {
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this transaction? This will reverse the cash/units.')) {
